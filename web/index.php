...
// Números exibidos no painel (consultas / repetidos).
$exibeConsultas = 248731;
$exibeRepetidos = 71482;
...
/** PAN completo agrupado de 4 em 4, para quando o número veio no lote atual. */
...
// O motor só devolve a máscara; o número completo vem do que foi colado agora.
// Chave = máscara, valor = PAN. Se dois PANs diferentes caírem na mesma máscara
// (mesmo BIN e mesmos 4 finais), nenhum dos dois é exibido completo.
$completos = [];
if ($resultado !== null) {
    foreach (preg_split('/\r\n|\r|\n/', $entrada) ?: [] as $linhaBruta) {
        foreach (AntiRetest::extractPans(trim($linhaBruta)) as $digitos) {
            $chave = AntiRetest::mask($digitos);
            if (array_key_exists($chave, $completos) && $completos[$chave] !== $digitos) {
                $completos[$chave] = null;
                continue;
            }
            $completos[$chave] = $digitos;
        }
    }
}
...
if ($resultado !== null) {
    ...
    foreach ($registros as &$reg) {
        $reg['completo'] = $reg['numero'] !== null ? ($completos[$reg['numero']] ?? null) : null;
    }
    unset($reg);
}
...
@media (max-width: 720px) {
    .topo-in, .app { padding-left: 16px; padding-right: 16px; }
    .barra-v, .nav-sep, .nav-pagina { display: none; }
    .topo-fim .topo-chip:nth-child(3) { display: none; }
    .hero-topo, .hero-corpo, .ed-wrap, .ed-pe, .p-head { padding-left: 14px; padding-right: 14px; }
    .hero-txt h1 { font-size: 22px; }
    .hero-sim { width: 44px; height: 44px; }
    .p-head { flex-wrap: wrap; gap: 10px; }
    .p-head-fim { margin-left: 0; flex: 1 1 100%; flex-wrap: wrap; }
    .seg { overflow-x: auto; flex: 1 1 auto; }
    .ed-pe { flex-wrap: wrap; }
    .pe-fim { margin-left: 0; flex: 1 1 100%; }
    .pe-fim .btn-pri { flex: 1; }
    .gutter { width: 40px; }
...
<header class="topo">
    <div class="topo-in">
        ...
        <div class="topo-fim">
            ...
            <?php if ($stats !== null): ?>
                <span class="topo-chip" data-tip="Consultas acumuladas">
            ...
            <div class="pop-wrap">
                ...
                <div class="pop" id="pop-avisos" hidden>
                    ...
                    <div class="aviso-item">
                        ...
                        <div><strong>Base com BIN e últimos 4</strong><span><?= $logDir === null
                            ? 'O número completo nunca é armazenado'
                            : 'A linha completa vai para o txt do dia' ?></span></div>
                    </div>
                </div>
            </div>
    ...
    <div class="colunas">
        ...
        <form method="post" id="form" class="painel">
            <div class="p-head">
                ...
                <div>
                    <h2>Entrada</h2>
                    <p>Informe a Store (ou vulgo) e cole as linhas (PAN, validade e CVV) para verificar contra a base.</p>
                </div>
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th class="c-vulgo">Adicionado por</th>
                    ...
                    <tbody id="corpo">
                        <?php foreach ($registros as $reg): ?>
                            ...
                            <tr data-situacao="<?= e($reg['situacao']) ?>">
                                <td>
                                    <div class="cel-num">
                                        ...
                                        <?php if ($reg['numero'] !== null): ?>
                                            <?php $inteiro = $reg['completo'] ?? null; ?>
                                            <span class="num"><?= $inteiro !== null
                                                ? numeroInteiro($inteiro)
                                                : numeroFormatado($reg['numero']) ?></span>
                                            <button type="button" class="copiar" aria-label="Copiar número"
                                                    data-copia="<?= e($inteiro ?? numeroPlano($reg['numero'])) ?>">
                                ...
                                <td><span class="selo <?= e($situacao['tom']) ?>"><?= e($situacao['rotulo']) ?></span></td>
                                <td>
                                ...
                                <td>
                                    <?php if (!empty($reg['vulgo'])): ?>
                                        <strong class="vulgo-nome"><?= e((string) $reg['vulgo']) ?></strong>
                                    <?php else: ?>
                                        <span class="nulo">—</span>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td class="t-consultas"><?= $reg['consultas'] !== null
...
(function () {
    ...
    function medir() {
        ...
        for (var i = 0; i < todas.length; i++) {
            ...
            if (!vazia) { uteis++; if (faixa !== null) { validos++; } }
