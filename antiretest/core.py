...
_ImportRow = tuple[str, tuple[str, str, str, str, str, str, int] | None, bool]
...
def fingerprint(digits: str, key: bytes) -> str:
    """Hash com chave: o número completo nunca é persistido."""
...
class AntiRetest:
        ...
        if row is None:
            if record:
                ...
                if self._log is not None:
                    # No txt vai a linha como veio; o banco fica só com o PAN.
    ...
    def _flush_import_batch(self, batch: list[_ImportRow], moment: datetime) -> tuple[int, int]:
        ...
        unique: dict[str, tuple[str, tuple[str, str, str, str, str, str, int, int]]] = {}
        for line, row in validas:
            unique.setdefault(row[0], (line, row))

        # Quais são novos só dá para saber antes do INSERT OR IGNORE; a consulta
        # extra só acontece quando há txt diário para alimentar.
        novos: set[str] = set()
        if self._log is not None:
