...
class AntiRetestTest(unittest.TestCase):
    ...
    def test_extra_fields_are_not_stored(self):
        self.engine.check(LINHA, now=BASE)
        rows = self.engine._conn.execute("SELECT * FROM entries").fetchall()
        dump = " ".join(str(value) for row in rows for value in tuple(row))
        self.assertNotIn("2028", dump)
    ...
    def test_plaintext_number_is_never_stored(self):
        result = self.engine.check(LINHA, now=BASE)
        self.assertNotIn(VALID, result.fingerprint)
        rows = self.engine._conn.execute("SELECT * FROM entries").fetchall()
        self.assertNotIn(VALID, " ".join(str(value) for row in rows for value in tuple(row)))
